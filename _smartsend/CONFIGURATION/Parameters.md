...
|  **Folder name**  | Folder name to save the file. This field can be left blank to create all files under the primary blob container. Placeholder tags can be used in this field to dynamically create folders. <br/> <br/> For example, you can create a folder based on the created date using %CreatedDate%. A backslash can be used to nest folders. <br/> <br/> The following will create a folder with the created date and the customer number. <br/> E.g. %CreatedDate%\%AccountNum% <br/> 20180622\C12345 <br/>  <br/> Refer to [Email Placeholder](Email_placeholder) for further detail on supported placeholder tags.|
...
### PDF Security
PDF Security allows for the password protection of both the view and edit properties of the PDF File generated by FSCM.
...
|  **Field**  | **Description** | 
|:---|:---|     
|User password|The User password can be used in conjunction with the Owner Password and provides the ability to restrict the view of the PDF document. <br> Note: It is recommended that the user password be provided to prevent all editing. Without a user password, online PDF conversion to other formats can still take place  |  
|Owner password|The owner password is mandatory for use with PDF Security and must be provided to prevent advanced editing of the document via PDF editing software <br> Note: The owner password does not prevent online conversion to other formats, as these do not require write permission of the document  |  
