...
On the Smart Send tab of the Email parameters form, you can configure the type of business form or report that should be sent using the Smart Send functionality.
...
|  **File name**  | File name that will be generated for the business form or report. Placeholder tags are supported. This helps with giving meaningful names to the files. Refer to [Email placeholder](Email_placeholder) for further details on supported placeholder tags. |  
...
|  **Folder name**  | Folder name to save the file. This field can be left blank to create all files under the primary blob container. Placeholder tags can be used in this field to create folders dynamically. <br/> <br/> For example, you can create a folder based on the created date using %CreatedDate%. A backslash can be used to nest folders. <br/> <br/> The following will create a folder with the created date and the customer number. <br/> E.g. %CreatedDate%\%AccountNum% <br/> 20180622\C12345 <br/>  <br/> Refer to [Email Placeholder](Email_placeholder) for further detail on supported placeholder tags.|
|  **Overwrite**  | If a file exists with the same name, it will only be overwritten if this field is ticked. |
|**Batch Processing. (selected forms only)**  | Activate Batch Processing of the selected form.<br>The option to process forms in batches will be enabled by setting the Radio Button under Allow batch processing to *Yes*.  This can enable the processing of several selections in the background without interfering with the user's ongoing interaction.  The "Smart Send" button initiates the batch process (*NOTE:* this activates a document-processing control that facilitates the assembly and distribution of the document via Smart Send and is separate from the standard FSCM Batch process that appears on print destination menus). The user is then presented with the batch processing form, which they must complete with the relevant information to queue the job for the batch run, as part of standard Batch processing functionality. <br> Note: Due to the unpredictable nature of Electronic Reporting (ER) in its current evolution. extensive testing should be carried out to ascertain the suitability of the process and consider any alterations to the current product design. before any deployment to a production environment  |


### PDF Security
PDF Security allows for the password protection of both the view and edit properties of the PDF File generated by FSCM and is specified per individual form.

PDF Security is controlled via Feature Management. Enable the Feature Key *DXC PDF security* under Feature management

Navigate to **System administration > Email > Email parameters > Smart send** Enable to *PDF security* radio button, populate your require password and select *Save*

When you enable the PDF Security radio button, you will be prompted with 2 password options

|  **Field**  | **Description** | 
|:---|:---|     
|User password|The User password can be used in conjunction with the Owner Password and provides the ability to restrict the view of the PDF document. <br> Note: It is recommended that the user password be provided in conjunction with the Owner password to prevent all editing. Without a user password, online PDF conversion to other formats (e.g., Word or Excel) can still take place; however, in-document editing is not possible |  
