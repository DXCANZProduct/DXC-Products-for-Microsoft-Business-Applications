...
The items status controls whether a product can be purchased, sales order (create a sales order), sales picking (create a pick list or output order) or sales return (allow sales returns) within the Dynamics 365 environment . In addition, the status can be used by the business to determine where in its lifecycle the product is. The item status can be held at the item or item dimension level. The item status can be manually updated via the released products form. 
