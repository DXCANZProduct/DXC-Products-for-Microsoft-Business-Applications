...
ms.search.form: SysEmailTable, SysEmailParameters
...
|  **Sales Order Confirmation**  | Confirmation Id: %ConfirmationId% <br/> Sales Id: %SalesId% <br/> Invoice Account: %InvoiceAccount% <br/> Delivery Name: %DeliveryName% <br/> Confirmation Date: %ConfirmDate% <br/> Purchase Order: %PurchaseOrder% <br/> Created Date: %CreatedDate% <br/> Customer Name: %CustomerName% <br/> Company Name: %CompanyName%| 
...
|  **Company Logo**  | The company logo can be incorporated into an organisational email template by incorporating an image from an online storage location, eg Sharepoint or Blob Stoarge into a placeholder command. Note: Approriate permissions need to be granted for FinOps to access to the image, commonly referred to as "anonymous" access . To set your placeholder insert your URL into <img src="*URL*"width="50" height="50" alt="logoImage"/> where "*URL*" is the location of your image.  You can adjust the image size by increasing the width and hieght values  specified at the end of the plcaeholder string 

as set in Organisation Administration can be included in the body of the email however you must use Tag `<img src="data:image/bmp;base64,%CompanyLogo%" alt="logo"/>` to render image correctly.<br> Note: the Image itself will not be visible inside D365 only in external Email Program eg Outlook
