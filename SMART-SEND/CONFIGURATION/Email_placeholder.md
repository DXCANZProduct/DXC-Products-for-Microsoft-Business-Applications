...
|  **Company Logo**  | The company logo can be incorporated into an organisational email template by incorporating an image from an online storage location, eg Sharepoint or Blob Stoarge into a placeholder command. Note: Approriate permissions need to be granted for FinOps to access to the image, commonly referred to as "anonymous" access . To set your placeholder insert your URL into <img src="*URL*"width="50" height="50" alt="logoImage"/> where "*URL*" is the location of your image.  You can adjust the image size by increasing the width and hieght values  specified at the end of the plcaeholder string.  The Image will render within organisational email templates if the template has been created with an appropriate html format. 
