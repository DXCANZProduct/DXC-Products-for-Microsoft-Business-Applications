...
|  **Blob container**  | The Container name that the document/report will be saved under.  This is a required field. <br> *Connection String* If a connection string is used, the value entered will be used to build the container if it doesn't already exist. <br> *SAS Token* Smart Send assumes that the container has already been built if an SAS token is being used.  If the container is not present in the Storage account, an error is raised.
