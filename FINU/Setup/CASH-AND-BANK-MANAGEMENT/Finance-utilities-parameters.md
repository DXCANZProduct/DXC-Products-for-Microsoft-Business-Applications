...
**Only match posted statement lines** | Select if the reconciliation matching rule used to create and post customer payment journals, should only included the bank statement lines that have been posted in matching. For example mark as new bank reconciliation rule finds three records in the bank statement, but only two of the customers exist and thus won't balance since only two lines were posted as customer payment journal lines. This option only applies to where the mark as new bank reconciliation creates and posts new customer payment journals.
