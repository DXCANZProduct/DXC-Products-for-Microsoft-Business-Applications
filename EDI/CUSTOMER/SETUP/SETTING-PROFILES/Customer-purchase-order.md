...
**Sales origin**  | If the target Sales order's **Sales origin** should be set to a specific value, select here. If field **Sales origin** is received in the import file, the staging value will be used to set the target Sales order's **Sales origin**.
