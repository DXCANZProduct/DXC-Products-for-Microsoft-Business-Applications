...
When the Version log displays an **Error type** of _Processing error_, the error hasn't been defined and the error **Message** will display the standard D365 error that stopped the processing. Same as manually processing, EDI will stop at the first processing error and only this error is displayed. Fixing the error and reprocessing might result in subsequent standard processing errors which need to be dealth with one by one.
