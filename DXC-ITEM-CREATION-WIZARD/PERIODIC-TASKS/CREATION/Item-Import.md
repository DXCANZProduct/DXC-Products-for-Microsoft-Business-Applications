...
Item import is a job that collects staged files imported into the *File management* workspace and creates an [import session](Creation-import-sessions) that will be automatically submitted to the active import [workflow](Item-creation-workflows). A *template name* should be selected when run to refine the files included in the import session. You can reach the Item import job by navigating to **Item creation > Periodic Tasks > Creation > Item import**.
